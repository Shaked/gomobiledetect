...
type route struct {
	re      *regexp.Regexp
	handler func(http.ResponseWriter, *http.Request, []string, *gomobiledetect.MobileDetect)
}

type RouterHandler struct {
	routes []*route
	detect *gomobiledetect.MobileDetect
}

func (h *RouterHandler) AddRoute(re string, handler func(http.ResponseWriter, *http.Request, []string, *gomobiledetect.MobileDetect)) {
	r := &route{regexp.MustCompile(re), handler}
	h.routes = append(h.routes, r)
}

func (h *RouterHandler) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	h.detect = gomobiledetect.NewMobileDetect(r, nil)
...
func homepageHandler(w http.ResponseWriter, r *http.Request, matches []string, detect *gomobiledetect.MobileDetect) {
