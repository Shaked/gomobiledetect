...
func (r *rules) getMobileDetectionRules() map[string]string {
...
func (rules *rules) setMobileDetectionRules() {
	mobileDetectionRules := make(map[string]string)
	for ruleName, ruleValue := range rules.phoneDevices {
		mobileDetectionRules[ruleName] = ruleValue
	}

	for ruleName, ruleValue := range rules.tabletDevices {
		mobileDetectionRules[ruleName] = ruleValue
	}

	for ruleName, ruleValue := range rules.operatingSystems {
		mobileDetectionRules[ruleName] = ruleValue
	}
